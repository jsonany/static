		         ...
		         function doubanid(id,type) {
					...
					success: function(data) {
									if (data.code == 1) {
										 ...
										 if (data.data.vod_pic) {
											$imgId = data.data.vod_pic.replace('.webp', '.jpg');
											if ($imgId != null) {
											$.ajax({
												type: "get",
												async: false,
												/* url: "https://api.longdidi.top/kkimg.php?url=" + $imgId, */
												url: "https://api.uomg.com/api/image.ali?imgurl=" + $imgId,
												dataType: "json",
												timeout: 5000,
												success: function(json) {
													if (json.code == 1) {
														layer.msg('获取远程图片成功', {icon:1,time: 1800});
														if (json.imgurl) {
															$('#vod_pic').val(json.imgurl); //获取图片连接
														}
													} else {
														layer.msg('获取远程图片失败', {icon:2,time: 1800});
				  ...
				  $(document).on("click", "#getimg", function() {
					$picurl = $('#vod_pic').val();
					$picurl = $picurl.replace('.webp', '.jpg');
					if ($picurl != null) {
						$.ajax({
							type: "get",
							async: false,
							/* url: "https://api.longdidi.top/kkimg.php?url=" + $picurl, */
							url: "https://api.uomg.com/api/image.ali?imgurl=" + $picurl,
							dataType: "json",
							timeout: 5000,
							success: function(json) {
								if (json.code == 1) {
									layer.msg('远程上传图片成功', {icon:1,time: 1800});
									if (json.imgurl) {
										$('#vod_pic').val(json.imgurl); //获取图片连接
									}
								} else {
									layer.msg('远程上传图片失败', {icon:2,time: 1800});
								}
							},
						});
					} else {
						layer.msg('请先填写图片地址', {icon:2,time: 1800});
					}
				  });
